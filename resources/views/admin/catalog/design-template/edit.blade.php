                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="template_name" class="form-control" id="template_name" value="{{ $designTemplate->name }}"
                                            required>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="template_status" required>
                                            <option {{ $designTemplate->status == 1 ? 'selected' : '' }} value="1">{{ __('Enable') }}</option>
                                            <option {{ $designTemplate->status == 0 ? 'selected' : '' }} value="0">{{ __('Disable') }}</option>
                                        </select>
                                        <div class="invalid-feedback">
                                ...
                                <div class="col-md-12">
                                    <ul class="nav nav-tabs product-layer-container" id="layerTabs" role="tablist">
                                        <button type="button" id="addLayerButton"
                                            class="btn btn-primary">{{ __('Add New Layer') }}</button>
                                    </ul>
                                    <div class="tab-content" id="layerContents">
                                    </div>
                                </div>
                                <div class="col-md-12 text-center">
                                    <button type="button" id="saveProductButton" class="btn btn-primary mt-3">
    ...
    <script>
        window.DesignTemplateConfig = {
            ...
            routes: {
                ...
                store: "{{ route('admin.catalog.design-template.update', $designTemplate->id) }}",
                index: "{{ route('admin.catalog.design-template.index') }}",
                deleteLayer: "{{ route('admin.catalog.design-template.layer.delete', ':id') }}",
            },
            baseUrl: "{{ Storage::url('/') }}",
            messages: @json(__('design_template')),
            template: @json($designTemplate->layers->values()),
        };
    </script>
    <script src="{{ asset('assets/js/pages/catalog/design-template/edit.js') }}"></script>
@endsection
