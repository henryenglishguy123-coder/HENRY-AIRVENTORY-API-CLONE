...
@section('title', __('Edit Discount Coupon'))

@section('content')
    <div class="page-breadcrumb">
        <div class="row align-items-center">
            <div class="col-12 d-flex justify-content-between">
                <h4 class="page-title">{{ __('Edit Discount Coupon') }}</h4>
    ...
    <div class="container-fluid">
        ...
        <x-alerts />
        <form action="{{ route('admin.marketing.discount-coupons.update', $discountCoupon->id) }}" method="POST" id="coupon_form"
            data-coupon-id="{{ $discountCoupon->id }}"
            ...
            data-lang-success-text="{{ __('Discount coupon updated successfully.') }}"
            ...
            data-preselected-products="{{ json_encode($preselectedProducts ?? []) }}"
            data-preselected-categories="{{ json_encode($preselectedCategories ?? []) }}"
            data-preselected-suppliers="{{ json_encode($preselectedSuppliers ?? []) }}"
            data-preselected-customers="{{ json_encode($preselectedCustomers ?? []) }}"
        >
            @csrf
            @method('PUT')
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="title" id="title"
                                class="form-control" value="{{ old('title', $discountCoupon->title) }}"
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <input type="text" name="code" id="code"
                                    ...
                                    value="{{ old('code',$discountCoupon->code)  }}" placeholder="SAVE20" required pattern="[A-Z0-9]+"
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="discount_type" id="discount_type"
                                ...
                                <option value="Order" 
                                    {{ old('discount_type', $discountCoupon->discount_type ?? '') == 'Order' ? 'selected' : '' }}>
                                    {{ __('Orders') }}
                                </option>
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <input type="number" step="0.01" name="amount_value" id="amount_value"
                                    ...
                                    value="{{ old('amount_value', $discountCoupon->amount_value ?? '') }}" min="0.01" required
        ...
        <div class="d-flex gap-4 mb-3 flex-wrap">
            <div class="form-check form-check-inline">
                <input type="radio" name="eligibility" value="All Customers" class="form-check-input"
                       id="eligibility_all"
                       {{ old('eligibility', $discountCoupon->eligibility ?? 'All Customers') == 'All Customers' ? 'checked' : '' }}>
            ...
            <div class="form-check form-check-inline">
                <input type="radio" name="eligibility" value="Specific Customers" class="form-check-input"
                       ...
                       {{ old('eligibility', $discountCoupon->eligibility ?? '') == 'Specific Customers' ? 'checked' : '' }}>
                <label class="form-check-label" for="eligibility_specific">{{ __('Specific Customers') }}</label>
            </div>
        </div>

        <!-- Specific Customers Select -->
        <div id="specific_customers_div" class="mt-3"
             style="display: {{ old('eligibility', $discountCoupon->eligibility ?? '') == 'Specific Customers' ? 'block' : 'none' }};">
            <label class="form-label required" for="customers">{{ __('Select Customers') }}</label>
            <select name="customers[]" class="form-select select2 @error('customers') is-invalid @enderror"
                    id="customers_select" multiple data-type="customer" placeholder="Search customers...">
                ...
                <div class="card-body">
                    <div class="d-flex gap-4 mb-3 flex-wrap">
                        <div class="form-check">
                            <input type="radio" name="min_requirement_type" value="none" id="min_req_none"
                                ...
                                {{ old('min_requirement_type', $discountCoupon->min_requirement_type) == 'none' ? 'checked' : '' }}>
                        ...
                        <div class="form-check">
                            <input type="radio" name="min_requirement_type" value="quantity" id="min_req_qty"
                                class="form-check-input"
                                {{ old('min_requirement_type', $discountCoupon->min_requirement_type) == 'quantity' ? 'checked' : '' }}>
                        ...
                        <div class="form-check">
                            <input type="radio" name="min_requirement_type" value="value" id="min_req_value"
                                class="form-check-input"
                                {{ old('min_requirement_type', $discountCoupon->min_requirement_type) == 'value' ? 'checked' : '' }}>
                    ...
                    <div class="row">
                        <div id="min_qty_group" class="mt-3 col-md-4"
                            style="display: {{ old('min_requirement_type', $discountCoupon->min_requirement_type ?? '') == 'quantity' ? 'block' : 'none' }};">
                            <label class="form-label required" for="min_qty">{{ __('Minimum Quantity') }}</label>
                            <input type="number" name="min_qty" id="min_qty"
                                class="form-control @error('min_qty') is-invalid @enderror"
                                value="{{ old('min_qty',$discountCoupon->min_qty ?? '') }}" min="1" placeholder="Enter minimum quantity">
                            @error('min_qty') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div id="min_price_group" class="mt-3 col-md-4"
                            style="display: {{ old('min_requirement_type', $discountCoupon->min_requirement_type ?? '') == 'value' ? 'block' : 'none' }};">
                            <label class="form-label required" id="min_price_input_label"
                                for="min_price">{{ __('Minimum Order Value') }}</label>
                            <input type="number" step="0.01" name="min_price" id="min_price"
                                ...
                                value="{{ old('min_price', $discountCoupon->min_price ?? '') }}" min="0.01" placeholder="Enter minimum price">
                            ...
                            <div class="row g-2" id="start_datetime">
                                <div class="col-6">
                                    <input type="date" name="start_date" id="start_date"
                                        ...
                                        value="{{ old('start_date', optional($discountCoupon->start_date)->format('Y-m-d')) }}">
                                    @error('start_date') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>
                                <div class="col-6">
                                    <input type="time" name="start_time" id="start_time"
                                        class="form-control @error('start_time') is-invalid @enderror" required
                                        value="{{ old('start_time', optional($discountCoupon->start_date)->format('H:i')) }}">
                            ...
                            <div class="row g-2" id="end_datetime">
                                <div class="col-6">
                                    <input type="date" name="end_date" id="end_date"
                                        ...
                                        value="{{ old('end_date', optional($discountCoupon->end_date)->format('Y-m-d')) }}">
                                    @error('end_date') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>
                                <div class="col-6">
                                    <input type="time" name="end_time" id="end_time"
                                        class="form-control @error('end_time') is-invalid @enderror"
                                        value="{{ old('end_time', optional($discountCoupon->end_date)->format('H:i')) }}">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <select name="status" id="status" class="form-select @error('status') is-invalid @enderror">
                                <option value="Active" {{ old('status', $discountCoupon->status ?? '') == 'Active' ? 'selected' : '' }}>
                                    {{ __('Active') }}
                                </option>
                                <option value="Inactive" {{ old('status', $discountCoupon->status ?? '') == 'Inactive' ? 'selected' : '' }}>
                                    {{ __('Inactive') }}
                                </option>
                            </select>
                            @error('status')
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="number" name="max_uses" id="max_uses"
                                ...
                                value="{{ old('max_uses', $discountCoupon->max_uses ?? '') }}" placeholder="Leave blank for unlimited total uses">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="max_uses_per_customer" id="max_uses_per_customer"
                                ...
                                value="{{ old('max_uses_per_customer', $discountCoupon->max_uses_per_customer ?? '') }}" min="1"
            ...
            <div class="text-end mb-5">
                <button type="submit" class="btn btn-primary">{{ __('Update Coupon') }}
                </button>
            </div>
        </form>
    </div>
@endsection
@section('js')
    <script src="{{ asset('assets/js/pages/marketing/discount-coupon/edit.js') }}"></script>
@endsection